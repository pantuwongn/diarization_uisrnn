...
class UISRNN:
    ...
    def fit_concatenated(self, train_sequence, train_cluster_id, args):
        ...
        for num_iter in range(args.train_iteration):
            ...
            nn.utils.clip_grad_norm_(
                self.rnn_model.parameters(), args.grad_max_norm)
            optimizer.step()
            ...
            if (np.remainder(num_iter, 10) == 0 or
                ...
                self.logger.print(
                    ...
                    'Regularization: {:.4f}'.format(
                        ...
                        float(loss2.data),
                        float(loss3.data)))
            # only save the likelihood part
            train_loss.append(float(loss1.data))
        self.logger.print(
    ...
    def predict_single(self, test_sequence, args):
        ...
        for num_iter in np.arange(0, args.test_iteration * test_sequence_length,
                                  args.look_ahead):
            max_clusters = max([len(beam_state.mean_set)
                                for beam_state in beam_set])
            ...
            for beam_rank, beam_state in enumerate(beam_set):
                beam_score_set = self._calculate_score(
                    beam_state, look_ahead_seq)
    ...
    def predict(self, test_sequences, args):
        ...
        raise TypeError(
            'test_sequences should be either a list or numpy array.')
